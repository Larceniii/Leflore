function getFormattedAddress(a) {
    var e = [
            ["house_number", "street_number"],
            ["house", "building", "public_building"],
            ["road", "footway", "street", "street_name", "residential", "path", "pedestrian", "road_reference", "road_reference_intl"],
...
function showPosition(i) {
    $.when($.ajax("https://nominatim.openstreetmap.org/reverse?format=json&lat=" + i.coords.latitude + "&lon=" + i.coords.longitude), $.ajax({
        type: "Post",
        url: "https://api.darksky.net/forecast/21950c0a7024a3f4bbb024e37d2bf163/" + i.coords.latitude + "," + i.coords.longitude + "?units=us&lang=en",
        beforeSend: function(e) {
            e && e.overrideMimeType && e.overrideMimeType("application/j-son;charset=UTF-8")
        },
        dataType: "jsonp",
        beforeSend: function() {
            console.log(i.coords.latitude)
        },
        complete: function() {
            console.log(i.coords.longitude)
        }
    })).done(function(e, t) {
        var n = e[0],
            o = t[0];
        console.log("showPosition", n);
        var a = setAddress(n);
        $(".cityVal").val(a[4]), $(".cityVal").next("span").toggle(Boolean($(".cityVal").val())), lati = i.coords.latitude, longt = i.coords.longitude, showWeather(o)
    })
}

function setAddress(e) {
    var t = getFormattedAddress(e.address);
    return console.log("Formatted:", t), $(".checkout__final-text").html(t[3]), PlaceName = t[4], t
}

function showWeather(e) {
    iconn = e.currently.icon, currTemp = e.currently.temperature.toString().replace(",", ".").split(".")[0] + "&deg;F / " + Math.round(toCelsius(e.currently.temperature.toString().replace(",", ".").split(".")[0])) + "&deg;C", summaryDay = e.daily.summary, getIconsKind();
    new Date(1e3 * parseInt(e.currently.time, 10));
    htmlCurrentWeather();
    for (var t = 0; t < e.daily.data.length; t++) iconn = e.daily.data[t].icon, inumber = t, summary = e.daily.data[t].summary, temper = "High: " + e.daily.data[t].temperatureMax.toString().replace(",", ".").split(".")[0] + "&deg;F / " + Math.round(toCelsius(e.daily.data[t].temperatureMax.toString().replace(",", ".").split(".")[0])) + "&deg;C", mintemper = "Low: " + Math.round(e.daily.data[t].temperatureMin) + "&deg;F / " + Math.round(toCelsius(e.daily.data[t].temperatureMin)) + "&deg;C", humidty = "Humidty: " + toPercent(e.daily.data[t].humidity), windspid = "Wind Speed: " + e.daily.data[t].windSpeed + " MPH / " + toKPH(e.daily.data[t].windSpeed) + " KPH", getIconsKind(), dday = new Date(1e3 * parseInt(e.daily.data[t].time, 10)), ddday = dday.toString().substring(0, 15), appendDaysList();
    getIconsPlay()
}
...
function getIconsPlay() {
    $(".skycons-element").each(function() {
        var e, t, n;
        return t = new Skycons({
            color: "#FFE066"
        }), e = $(this).attr("id"), n = $(this).data("skycons"), t.add(e, Skycons[n]), t.play()
    });
    var e = new Skycons;
...
function getMapPlace() {
    var e = {
        ...
        i = (new google.maps.InfoWindow, new google.maps.LatLngBounds, new google.maps.Map(document.getElementById("dvMap"), e));
    navigator.geolocation.getCurrentPosition(function(e) {
        var n = new google.maps.LatLng(lati, longt),
        ...
        google.maps.event.addListener(o, "dragend", function() {
            a.open(i, o), i.setCenter(o.getPosition())
...
function toKPH(e) {
    return (1.609344 * e).toFixed(2)
}
$(document).ready(function() {
        getLocation(), $(window).load(function() {
            getIconsPlay(), setTimeout(function() {
                $("#preloader").velocity({
                    opacity: .1,
                    translateY: "-80px"
                }, {
        ...
        $(".daysList").html(""), getIconsPlay(), getLocation()
    }), $(".getCity").on("click", function() {
        $(".daysList").html("");
        var e = $(".cityVal").val();
        $.ajax("https://nominatim.openstreetmap.org/search?q=" + e + "&format=json").done(function(e) {
            console.log(e);
            var a = e[0];
            $.when($.ajax("https://nominatim.openstreetmap.org/reverse?format=json&lat=" + a.lat + "&lon=" + a.lon), $.ajax({
                type: "Post",
                url: "https://api.darksky.net/forecast/7872f3201f558ed1217c55b409c43f75/" + a.lat + "," + a.lon + "?units=us&lang=en",
                beforeSend: function(e) {
                    e && e.overrideMimeType && e.overrideMimeType("application/j-son;charset=UTF-8")
                },
                dataType: "jsonp",
                ...
                var n = e[0],
                    o = t[0];
                setAddress(n);
                lati = a.lat, longt = a.lon, showWeather(o)
            })
        })
    }), $(".cityVal").keyup(function(e) {
        13 == e.keyCode && $(".getCity").click();
        var t = $(this);
        t.next("span").toggle(Boolean(t.val()))
    }), $("#searchclear").hide($(this).prev(".cityVal").val()), $("#searchclear").on("click", function() {
        $(this).prev(".cityVal").val("").focus(), $(this).hide()
    }), [].slice.call(document.querySelectorAll(".checkout")).forEach(function(t) {
        var e = t.querySelector(".checkout__button"),
            n = t.querySelector(".checkout__cancel");
        e.addEventListener("click", function(e) {
            e.preventDefault(), classie.add(t, "checkout--active")
        }), n.addEventListener("click", function() {
            classie.remove(t, "checkout--active")
        })
    }), $().ready(function() {
        $('[rel="tooltip"]').tooltip()
    }),
    ...
    }(window), $(".color-tool li a").on("click", function() {
